...
import { makeGetPaymentNotificationMetadataRouter } from '../router';
...
describe('Get Payment Notification Metadata Router', () => {
  const app = express();
  app.use(express.json());
  const router = makeGetPaymentNotificationMetadataRouter(mockEnv);
  app.use('/api', router);

  it('should return a 400 response with some data', async () => {
    const response = await supertest(app).get('/api/delivery/notifications/sent/1A/attachments/payment/2B/3C').query({
