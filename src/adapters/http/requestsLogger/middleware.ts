...
export const middleware =
  (env: SystemEnv): MiddlewareHandler =>
  async (req, res, next) => {
    ...
    if (baseUrl.startsWith('/uploadS3/')) {
      localCapturedResponse = localCapturedResponse.concat(
        `, and x-amz-version-id is: ${res.getHeader('x-amz-version-id')}`
      );
    }
    ...
    const requestResponseRecord = makeRequestResponseRecord(env)(
      E.isRight(inputValidation) ? inputValidation.right : inputValidation.left
    );
