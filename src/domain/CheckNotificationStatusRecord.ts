...
type PhysicalAddress = NonNullable<NotificationRecipientV24['physicalAddress']>;

const ensurePhysicalAddress = (rec: { physicalAddress?: PhysicalAddress | null }): PhysicalAddress => ({
  address: rec.physicalAddress?.address ?? '',
  municipality: rec.physicalAddress?.municipality ?? '',
  zip: rec.physicalAddress?.zip ?? '',
  at: rec.physicalAddress?.at,
  addressDetails: rec.physicalAddress?.addressDetails,
  municipalityDetails: rec.physicalAddress?.municipalityDetails,
  province: rec.physicalAddress?.province,
  foreignState: rec.physicalAddress?.foreignState,
});
          ...
          O.map((response) =>
            pipe(
              ...
              RA.reduce(response, function (res, invalidRec) {
                ...
                const phys = ensurePhysicalAddress(invalidRec as any);
                if (!VALID_CAPS[phys.zip as keyof typeof VALID_CAPS]) {
                  return t.exact(NewNotificationRequestStatusResponseV25).encode({
                    ...
                    errors: [
                      ...pastErrors,
                      {
                        ...
                        detail: `Validation failed, address is not valid. Error=Cap ${phys.zip} not found`,
                      },
                    ],
                  });
                }
              ...
              RA.reduce(response, (respAccrRaw, docRespRaw) => {
                ...
                const matchFound = RA.reduce(false, (corresponds, recordRaw) => {
                  ...
                  if (recordFull.type === 'PreLoadRecord') {
                    ...
                    const hasTokenOnePreload = RA.reduce(
                      false,
                      (hasTokenOnePreload, preloadRecordRaw) => {
            ...
            response.notificationRequestStatus === 'REFUSED'
              ? {
                  ...
                  returned: Object.assign(
                    ...
                    { ...(response as NewNotificationRequestStatusResponseV25), retryAfter: env.retryAfterMs / 1000 }
                  ),
                }
              : {
